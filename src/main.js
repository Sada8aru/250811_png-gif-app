...
const extractGifFrames = async (file, { fullFrame = true } = {}) => {
  ...
  if (fullFrame) {
    return await decodeGif(arrayBuffer); // フルフレーム展開
  } else {
    return await parseGifFrames(arrayBuffer); // 差分フレーム（今の処理）
  }
};

async function assembleFullFrames(
  frames,
  gifWidth,
  gifHeight,
  ...
  for (let i = 0; i < frames.length; i++) {
    ...
    // disposalType === 3 の場合は現在のフルキャンバスを保存
    if (f.disposalType === 3) {
    ...
    if (patchW > 0 && patchH > 0) {
      ...
      // できれば createImageBitmap を使って drawImage（アルファ合成される）
      if (typeof createImageBitmap === "function") {
        const bitmap = await createImageBitmap(imageData);
        ctx.drawImage(bitmap, f.dims.left, f.dims.top);
        bitmap.close?.(); // 一部環境でリソース解放
      } else {
        // createImageBitmap が無い場合のフォールバック：一時キャンバスに putImageData してから drawImage
    ...
    // 今の canvas がフルフレームになっているので Image を作る
    ...
    result.push({
      image: frameImage,
      delay: f.delay ?? 100,
      disposal: f.disposalType ?? f.disposal,
    });

    // disposal を適用（次フレームへ影響）
    switch (f.disposalType) {
      case 2: // restore to background (clear the rect or fill with backgroundColor)
        if (backgroundColor !== null) {
          ctx.fillStyle = backgroundColor;
        ...
        break;
      case 3: // restore to previous (put saved previousImageData back)
...
const decodeGif = async (arrayBuffer) => {
  ...
  // buildPatch = true (RGBA patch) を取得
  ...
  // 既存 parseGifFrames と同じ形式に揃える
  return fullFrames.map((f) => ({
    image: f.image,
    delay: f.delay, // ms 単位に合わせていることを確認
    disposal: f.disposal,
  }));
};
...
const parseGifFrames = async (arrayBuffer) => {
  try {
    ...
    for (let i = 0; i < reader.numFrames(); i++) {
      ...
      console.log(
        `フレーム ${frameInfo.disposal} ${
          i + 1
        }/${reader.numFrames()} 抽出完了 (delay: ${frameInfo.delay * 10}ms)`
      );
    }
...
const handleCropDrag = (e) => {
  ...
  if (isCropDragging) {
    ...
    if (aspectRatio === "free") {
      ...
      const newX = Math.max(
        0,
        Math.min(bg.metadata.width - newWidth, centerX - newWidth / 2)
      );
      const newY = Math.max(
        0,
        Math.min(bg.metadata.height - newHeight, centerY - newHeight / 2)
      );
...
const exportAsGIF = () => {
  ...
  try {
    ...
    // Vite でワーカーを正しく配信するため、import.meta.url を使って相対パスを解決します
    ...
    const gif = new GIF({
      ...
      height: canvasHeight,
      repeat: 0, // 無限ループ
      workerScript: workerScriptUrl, // Vite 互換のワーカースクリプトURLを指定
    });
    ...
    projectState.transparentImages.forEach((transparentImg, index) => {
      try {
        ...
        console.log(
          `フレーム ${index + 1}/${
            projectState.transparentImages.length
          } 追加完了`
        );
      } catch (frameError) {
